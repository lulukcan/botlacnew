	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im LAC BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                aruga.reply(from, 'Gw minta tolong bgt kak, tlong difollow ig saya, susahnya follow doang apa? 👊🙁 https://www.instagram.com/bknsr mau difollowback DM saja')
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'GAUSAH MINTA BUAT STIKER KALAU GA FOLLOW IG SAYA 👊🙁 https://www.instagram.com/bknsr')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                aruga.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    aruga.reply(from, 'GAUSAH MINTA BUAT STIKER KALAU GA FOLLOW IG SAYA 👊🙁 https://www.instagram.com/bknsr')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, 'Ini kak! Jangan lupa follow ig gw ya 👊🙁 https://www.instagram.com/bknsr',id)
                    })
                    .catch(() => {
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                try {
                    ...
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Jangan lupa follow ig gw ya 👊🙁 https://www.instagram.com/bknsr', id)
                } catch {
            ...
            break
        case 'nulis':
            if (args.length == 0) return aruga.reply(from, `Membuat bot menulis teks yang dikirim menjadi gambar\nPemakaian: ${prefix}nulis [teks]\n\ncontoh: ${prefix}Jangan lupa follow ig gw ya https://www.instagram.com/bknsr`, id)
            const nulisq = body.slice(7)
            const nulisp = await rugaapi.tulis(nulisq)
            await aruga.sendImage(from, `${nulisp}`, '', 'Success...Jangan lupa follow ig gw ya 👊🙁 https://www.instagram.com/bknsr', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', 'Success..Jangan lupa follow ig gw ya 👊🙁 https://www.instagram.com/bknsr', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomkpopx, '', 'Success..Jangan lupa follow ig gw ya 👊🙁 https://www.instagram.com/bknsr', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot LAC sebagai admin grup!', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *L U L U K  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *L U L U K B C* 〙\n\n${msg}`)
            }
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            ...
            for (let gclist of allGroupz) {
                await aruga.sendText(gclist.contact.id, `Maaf bot sedang pembersihan kak, total chat aktif : ${allChatz.length}`)
