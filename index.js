...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('LAC BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Luluk', 'yellow'))
	...
	await aruga.sendText(chat.id, `Sorry, the group on this bot is full, jika pengen bot ini masuk group silahkan WA: 089660728860\nMax Group is: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai minna~, Im LAC BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Hello 👏, Welcome to di group ini kaka😇 @${event.who.replace('@c.us', '')}\nJangan lupa follow ig saya ya https://instagram.com/bknsr/ ✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Good bye, semoga anda tenang disana & diterima di neraka yaa👍😁 @${event.who.replace('@c.us', '')}, We'll miss you✨`)
        }
    })

    aruga.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await aruga.sendText(callData.peerJid, 'Terimakasih anda telah melanggar rulles BoT saya dengan menelfon BoT ini, Anda gw block! 👍.\n\n-BOT LAC 🤖')
        .then(async () => {
